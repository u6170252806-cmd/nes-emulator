...
Mapper009::Mapper009(uint8_t prg_banks, uint8_t chr_banks) 
...
bool Mapper009::cpu_read(uint16_t addr, uint8_t& data, uint8_t* prg_rom) {
    if (addr >= 0x8000 && addr <= 0x9FFF) {
        // Switchable 8KB PRG ROM bank
        uint32_t mapped_addr = prg_bank * 0x2000 + (addr & 0x1FFF);
        data = prg_rom[mapped_addr];
        return true;
    } else if (addr >= 0xA000 && addr <= 0xFFFF) {
        // Fixed 24KB PRG ROM (last 3 banks)
        uint32_t mapped_addr = (prg_banks * 2 - 3) * 0x2000 + (addr & 0x5FFF);
        data = prg_rom[mapped_addr];
        return true;
    }
    return false;
...
bool Mapper009::cpu_write(uint16_t addr, uint8_t data, uint8_t* prg_rom) {
...
bool Mapper009::ppu_read(uint16_t addr, uint8_t& data, uint8_t* chr_rom) {
...
bool Mapper009::ppu_write(uint16_t addr, uint8_t data, uint8_t* chr_rom) {
    // CHR ROM is read-only
    return false;
}

Mapper::Mirror Mapper009::get_mirror() {
    return mirror;
}
